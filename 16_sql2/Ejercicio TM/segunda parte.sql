...
-- 5.
SELECT distinct actors.first_name, actors.last_name FROM movies JOIN (actor_movie JOIN actors ON actor_movie.actor_id = actors.id) 
ON movies.id = actor_movie.movie_id WHERE movies.title LIKE "%guerra de las galaxias%";