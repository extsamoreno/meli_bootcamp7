...
public class UserController {
    ...
    public ResponseEntity<FollowersListDTO> followersList (@PathVariable int userId, @RequestParam(required = false) String order) {
        return new ResponseEntity<FollowersListDTO>( iUserService.getFollowerList(userId, order),HttpStatus.OK);
    }
    @GetMapping("/{userId}/followed/list")
    public ResponseEntity<FollowedListDTO> followedList (@PathVariable int userId, @RequestParam(required = false) String order) {
